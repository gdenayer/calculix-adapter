                  ...
                  char *preciceParticipantName, char *configFilename)
{
  ...
  *nzl = 0;
  ...
  if (!cyclicsymmetry) {
    ...
    if (iperturb[0] == 1) {
      ...
      SFREE(vold);
      NNEW(vold, double, mt **nk *nsectors);
    }
  ...
  *tmax = *tmax / (*tper);
  ...
  while (Precice_IsCouplingOngoing()) {
      ...
      if (iabsload != 2) {
        if (nactmech < neq[1] / 2) {
          for (i = 0; i < nactmech; i++) {
            if (fabs(fext[ikactmech[i]]) > fextmax)
              fextmax = fabs(fext[ikactmech[i]]);
          }
        } else {
          for (i = 0; i < neq[1]; i++) {
            if (fabs(fext[i]) > fextmax)
              fextmax = fabs(fext[i]);
          }
        }
      } else {
      ...
      for (l = 0; l < nev; l++) {
        ...
        /*   subcritical damping */
          ...
          bj[l] = sum / ddj + fexp * (fcos + zetaj / sqrt(1. - zetaj * zetaj) * fsin) * cd[l] +
                  fexp * fsin * cv[l] / ddj;
          bjp[l] = sump / ddj + fexp * ((-h1 + ddj * h14) * fcos + (-ddj - h1 * h14) * fsin) * cd[l] + fexp * (-h1 * fsin + ddj * fcos) * cv[l] / ddj;
        ...
        /*      supercritical damping */
          ...
          fexp   = exp(-h2 * dtime);
          h14    = zetaj * dj / ddj;
          bj[l]  = sum / (2. * ddj) + (fexm + fexp) * cd[l] / 2. + zetaj * (fexm - fexp) / (2. * sqrt(zetaj * zetaj - 1.)) * cd[l] + (fexm - fexp) * cv[l] / (2. * ddj);
          bjp[l] = sump / (2. * ddj) + (h1 * fexm - h2 * fexp) * cd[l] / 2. + (h14 * cd[l] + cv[l] / ddj) * (h1 * fexm + h2 * fexp) / 2.;
        }
    ...
    if (precicec_isTimeWindowComplete()) {
      ...
      while (BufferCanRead(out_buffer)) {
        BufferLoadDouble(out_buffer, "ptime", &ptime);
        BufferLoadDouble(out_buffer, "veold", veold);
        BufferLoadDouble(out_buffer, "vold", vold);
        BufferLoadDouble(out_buffer, "v", v);
        BufferLoadITG(out_buffer, "iinc", &iinc);
        BufferLoadITG(out_buffer, "kode", kode);
  ...
  SFREE(xforcdiff);
  SFREE(xloaddiff);
  SFREE(xboundiff), SFREE(xbodydiff);
